...
import cn.yuencode.flowlongplus.service.impl.FileUploadExtService;
...
public class OssController {
    ...
    @Resource
    private FileUploadExtService fileUploadExtService;
//
//    @PostMapping("/upload")
//    @SaCheckLogin
...
//    }
    ...
    @SaCheckLogin
    public R<?> upload(@RequestPart MultipartFile file) {
        FileUploadInfo fileUploadInfo=  fileUploadExtService.upload(file);
        return R.success(BeanUtil.beanToMap(fileUploadInfo, "id", "size", "filename"));
    }
...
//    }
    ...
    public void file(@PathVariable String id, @RequestParam(required = false, defaultValue = "false") Boolean download, HttpServletResponse response) {
        ...
        //判断文件是否存在
        boolean exists = fileUploadExtService.exists(sysFileDetail);
        if (!exists) {
            throw new CommonException("文件不存在");
        }

        byte[] bytes = fileUploadExtService.download(sysFileDetail);
        try {
            ServletOutputStream os = response.getOutputStream();
            String filename = sysFileDetail.getFilename();
//            MediaType mediaType = MediaTypeFactory.getMediaType(filename).orElse(MediaType.APPLICATION_OCTET_STREAM);
            MediaType mediaType = MediaType.parseMediaType(sysFileDetail.getContentType());
            response.setContentType(mediaType.toString());
            response.setContentLength(sysFileDetail.getSize().intValue());
