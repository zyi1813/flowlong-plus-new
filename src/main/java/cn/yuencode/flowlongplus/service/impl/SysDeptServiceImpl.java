...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements SysDeptService {
    ...
    public Object getAllDepts() {
        List<SysDept> sysDepts = baseMapper.selectList(Wrappers.<SysDept>lambdaQuery()
                .eq(SysDept::getIsEnable, true) // 只获取启用的部门
        ...
        for (SysDept sysDept : sysDepts) {
            ...
            map.put("id", sysDept.getDeptId());
            map.put("name", sysDept.getDeptName());
            map.put("type", "dept"); // 标识这是部门类型
            resp.add(map);
        }
        return resp;
    }
