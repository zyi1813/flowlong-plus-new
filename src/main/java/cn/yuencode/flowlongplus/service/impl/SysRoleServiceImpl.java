...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public Object getAllRoles() {
        List<SysRole> sysRoles = baseMapper.selectList(Wrappers.<SysRole>lambdaQuery()
                .eq(SysRole::getStatus, 1) // 只获取启用的角色
        ...
        for (SysRole sysRole : sysRoles) {
            ...
            map.put("code", sysRole.getRoleCode());
            map.put("type", "role"); // 标识这是角色类型
            resp.add(map);
        }
        return resp;
    }
    ...
    @Override
    public Object getUsersByRole(String roleId) {
        // 根据角色ID查询用户列表
        List<Map<String, Object>> users = sysUserRoleMapper.selectUsersByRoleId(roleId);
        List<Map<String, Object>> resp = new ArrayList<>();
        for (Map<String, Object> user : users) {
            Map<String, Object> map = new HashMap<>();
            map.put("id", user.get("id"));
            map.put("name", user.get("nickname"));
            map.put("type", "user"); // 标识这是用户类型
            resp.add(map);
        }
        return resp;
    }
